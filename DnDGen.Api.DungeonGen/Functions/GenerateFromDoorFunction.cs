...
{
    public class GenerateFromDoorFunction
    {
        private readonly ILogger _logger;
        private readonly IEncounterVerifier _verifier;
        private readonly IDungeonGenerator _generator;

        public GenerateFromDoorFunction(ILoggerFactory loggerFactory, IDependencyFactory dependencyFactory)
        {
            _logger = loggerFactory.CreateLogger<GenerateFromDoorFunction>();
        ...
        public async Task<HttpResponseData> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "v1/dungeon/level/{dungeonLevel:int}/door/{temperature}/{environment}/{timeOfDay}/level/{partyLevel:int}/generate")] HttpRequestData req,
            int dungeonLevel, string temperature, string environment, string timeOfDay, int partyLevel)
        {
            _logger.LogInformation("C# HTTP trigger function (GenerateFromDoorFunction.Run) processed a request.");
