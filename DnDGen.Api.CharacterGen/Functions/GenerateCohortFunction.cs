...
namespace DnDGen.Api.CharacterGen.Functions
{
    public class GenerateCohortFunction
    {
        ...
        [OpenApiOperation(operationId: "GenerateCohortFunctionRun", Summary = "Generate cohort",
            Description = "Generate a cohort for a leader at the given level and cohort score.")]
        [OpenApiParameter(name: "leaderLevel", In = ParameterLocation.Path, Required = true, Type = typeof(int),
        ...
        public Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "v1/leadership/level/{leaderLevel}/cohort/{cohortScore}/generate")] HttpRequest req,
            int leaderLevel, int cohortScore, ILogger log)
        {
            log.LogInformation("C# HTTP trigger function (GenerateCohortFunction.Run) processed a request.");

            var validatorResult = CohortValidator.GetValid(leaderLevel, req);
            ...
            log.LogInformation($"Generated Cohort: {cohort.Summary}");
